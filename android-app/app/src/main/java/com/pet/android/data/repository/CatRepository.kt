...
class CatRepository @Inject constructor(
    ...
    suspend fun getCatById(id: String): Resource<CatRequest> {
        return withContext(Dispatchers.IO) {
            try {
                val response = catApi.getCatById(id)
                if (response.success && response.data != null) {
                    Resource.Success(response.data)
                } else {
                    Resource.Error(response.message ?: "取得貓咪資料失敗")
                }
            } catch (e: Exception) {
                Resource.Error(e.message ?: "網路錯誤")
            }
    ...
    suspend fun updateCat(id: String, cat: CatRequest): Resource<CatRequest> {
        return withContext(Dispatchers.IO) {
            try {
                val response = catApi.updateCat(id, cat)
                if (response.success && response.data != null) {
                    Resource.Success(response.data)
                } else {
                    Resource.Error(response.message ?: "更新貓咪失敗")
                }
            } catch (e: Exception) {
                Resource.Error(e.message ?: "網路錯誤")
            }
